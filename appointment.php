	...
	<div class="container">
		...
		if (isset($_POST['submit_book_appointment_form']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			try {
				...
				if ($client_count > 0) {
					...
					$stmtgetCurrentClientID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'barberia' AND TABLE_NAME = 'clients'");
				...
				$stmtgetCurrentAppointmentID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'barberia' AND TABLE_NAME = 'appointments'");
		...
		<form method="post" id="appointment_form" action="appointment.php">
			...
			<div class="select_date_time_div tab_reservation" id="calendar_tab">
				...
				<div class="calendar_tab" style="overflow-x: auto;overflow-y: visible; background-color:#8CFFFB" id="calendar_tab_in">
			...
			<div style="overflow:auto;padding: 30px 0px;">
				<div style="float:right;">
					...
					<button type="button" id="prevBtn" class="next_prev_buttons" style="background-color: #fff;" onclick="nextPrev(-1)">Previo</button>
